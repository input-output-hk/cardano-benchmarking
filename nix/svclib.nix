...
### TODO:
###
###  1. add support for environments
###
...
let
  ...
  mkServiceScript =
    ...
    let cmd = map normaliseCmdArg ([
          "${package.components.exes.${exeName}}/bin/${exeName}"
        ...
        normaliseCmdArg = x:
          ({ int = toString; float = toString; string = id;
            ...
            (_: throw "\nWhile processing service declaration for '${svcName}':  Unsupported type of command arg: ${typeOf x}\n")) x;
    in  ''
...
{ inherit
  ...
  scripts = mapAttrs processScriptDeclaration (import ./scripts.nix);
}
