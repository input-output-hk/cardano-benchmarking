...
FIRSTSLOT=`jq -r 'select(.data.event.kind=="TraceAddBlockEvent.AddedToCurrentChain") | [ .at, .data.event.kind, .data.event.newtip ] | @csv' $LOGFILE_JSON | sed -e 's/"[a-f0-9]\+@\([0-9]\+\)"/\1/;' | sort -k 1.2,1.23 | head -n 1 | sed -e 's/^\("[0-9]\+-[0-9]\+-[0-9]\+\)T\([0-9]\+:[0-9]\+:[0-9]\+[.][0-9]\+\)Z\(.*\)$/\1 \2\3/'`

LOGFILE_JSON=`ls -1r state-node-${NETWORK}/node-0-*.json | head -2`
echo $LOGFILE_JSON
LASTSLOT=`jq -r 'select(.data.event.kind=="TraceAddBlockEvent.AddedToCurrentChain") | [ .at, .data.event.kind, .data.event.newtip ] | @csv' $LOGFILE_JSON | sort -k 1.2,1.23 | tail -1 | sed -e 's/"[a-f0-9]\+@\([0-9]\+\)"/\1/;s/^\("[0-9]\+-[0-9]\+-[0-9]\+\)T\([0-9]\+:[0-9]\+:[0-9]\+[.][0-9]\+\)Z\(.*\)$/\1 \2\3/'`
