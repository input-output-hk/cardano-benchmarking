...
echo $LOGFILE_JSON
LASTSLOT=`jq -r 'select(.data.kind=="TraceAddBlockEvent.AddedToCurrentChain") | [ .at, .data.kind, .data.newtip ] | @csv' $LOGFILE_JSON | sort -k 1.2,1.23 | tail -1 | sed -e 's/"[a-f0-9]\+@\([0-9]\+\)"/\1/;s/^\("[0-9]\+-[0-9]\+-[0-9]\+\)T\([0-9]\+:[0-9]\+:[0-9]\+[.][0-9]\+\)Z\(.*\)$/\1 \2\3/'`
