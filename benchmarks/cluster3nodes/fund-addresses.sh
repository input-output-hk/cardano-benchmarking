...
run 'cardano-cli' shelley query protocol-parameters \
    --testnet-magic $MAGIC \
...
do
    ...
    # TTL=$(run 'cardano-cli' shelley query tip --testnet-magic $MAGIC) + 250

    # FEE=$(run 'cardano-cli' shelley transaction calculate-min-fee \
    #         --tx-in-count 1 \
    #         --tx-out-count 2 \
    #         --ttl $TTL \
    #         --testnet-magic $MAGIC \
